...
if ( ! class_exists( 'Spice_Starter_Sites_Importer_Auto' ) ) {
    ...
    // Increase memory limit safely
    wp_raise_memory_limit( 'import' );

    /**
     * rewrite_urls does NOT exist in auto imports.
     * Must be guarded for PHP 8+
     */
    $rewrite_urls = false;

    if ( isset( $_POST['rewrite_urls'] ) ) {
		...
		function process_posts() {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( ! post_type_exists( $post['post_type'] ) ) {
					printf(
						__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'spice-starter-sites' ),
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					printf( __( '%1$s &#8220;%2$s&#8221; already exists.', 'spice-starter-sites' ), $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
					echo '<br />';
					...
					if ( ! empty( $this->options ) && ! empty( $this->options['rewrite_urls'] ) ) {
						$url_mapping              = array(
					...
					if ( is_wp_error( $post_id ) ) {
						printf(
							__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'spice-starter-sites' ),
	...
	protected function process_post_term( $term, $post_id, $post ) {
		...
		if ( ! $term_id ) {
			...
			if ( is_wp_error( $t ) ) {
				printf( __( 'Failed to import %1$s %2$s', 'spice-starter-sites' ), esc_html( $taxonomy ), esc_html( $term['name'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
